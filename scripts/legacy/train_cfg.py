#!/usr/bin/env python
"""
UNet 训练脚本（支持 CFG 和 CBCT初始化）

用法:
    # 训练带CFG和CBCT初始化的模型
    python scripts/train_cfg.py --arch concatenation --epochs 200 \
        --use-cfg --cfg-dropout-rate 0.15 \
        --use-cbct-init --alpha-a 0.5

    # 训练不带CFG的模型（baseline）
    python scripts/train_cfg.py --arch concatenation --epochs 200

    # 快速测试
    python scripts/train_cfg.py --arch concatenation --epochs 5 --quick-test \
        --use-cfg --use-cbct-init

    # 禁用 wandb
    python scripts/train_cfg.py --arch concatenation --no-wandb --use-cfg
...
def train_with_wandb(
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if val_loss < best_val_loss:
            ...
            # 只保存unet_best.pth（覆盖更新，不保存带epoch标签的best）
            unet_best_path = save_path.replace('.pth', '_best.pth')
            torch.save(state_dict_cpu, unet_best_path)

            save_message = f"✅ Best model at epoch {epoch+1}, val_loss={val_loss:.4f} (saved: unet_best.pth)"
            print(save_message)
...
def train_architecture(arch_name, args, wandb_logger=None):
    ...
    # 生成带时间戳的实验目录名 (格式: arch_CFG_timestamp_ep200)
    # 注意：CBCT初始化只在推理时使用，不影响训练目录命名
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    cfg_suffix = "CFG" if args.use_cfg else "noCFG"
    exp_dir_name = f"{arch_name}_{cfg_suffix}_{timestamp}_ep{args.epochs}"
