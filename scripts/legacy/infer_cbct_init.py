...
核心思想：不从纯噪声(t=999)开始，而是从中间步(t=500等)的CBCT+噪声开始
优势：
1. 加速采样（跳过高噪声阶段）
2. CBCT提供结构先验
3. 利用CBCT≈CT的特性
...
用法:
    # 标准推理（从t=999开始）
    python scripts/infer_cbct_init.py --vae vae.pth --unet unet.pth --input test/ --t-start 999

    # 中间步初始化（从t=500开始，更快）
    python scripts/infer_cbct_init.py --vae vae.pth --unet unet.pth --input test/ --t-start 500 --use-cbct-init

    # 更激进的加速（从t=300开始）
    python scripts/infer_cbct_init.py --vae vae.pth --unet unet.pth --input test/ --t-start 300 --use-cbct-init --ddim-steps 20
"""
import os
import sys
import argparse
...
def parse_args():
    ...
    # 中间步初始化参数（核心创新点）
    parser.add_argument('--t-start', type=int, default=999,
                        help='起始时间步 (0-999)。越小=噪声越少=采样越快。'
                             '建议值: 999(标准), 700(快速), 500(更快), 300(激进)')
...
def get_timesteps(total_steps, num_steps, schedule, t_start=None):
    """生成时间步序列
    ...
    Args:
        ...
        num_steps: 采样步数
        schedule: 调度策略
        t_start: 起始时间步（None则使用total_steps-1，即999）
    ...
    if schedule == "linear":
        # 从t_start到0，均匀分布num_steps个点
        return list(np.linspace(0, t_start, num_steps, dtype=int)[::-1])
    elif schedule == "quadratic":
        t = np.linspace(0, 1, num_steps) ** 2
        return list((t * t_start).astype(int)[::-1])
    elif schedule == "power":
        coarse = num_steps // 3
        fine = num_steps - coarse
        coarse_t = np.linspace(t_start, t_start * 0.3, coarse)
        fine_t = np.linspace(t_start * 0.3, 0, fine)
        return list(np.concatenate([coarse_t, fine_t]).astype(int))
    else:
        step_size = max(1, t_start // num_steps)
        return list(range(t_start, -1, -step_size))
...
def main():
    ...
    # 生成时间步（支持中间步初始化）
    timesteps = get_timesteps(1000, args.ddim_steps, args.schedule, t_start=args.t_start)
    print(f"  时间步: t_start={args.t_start}, steps={args.ddim_steps}")
    print(f"  timesteps范围: [{timesteps[0]} → {timesteps[-1]}]")
    ...
    for batch_idx, (ct_gt, cbct) in enumerate(tqdm(loader, desc="推理中")):
        ...
        for i in range(batch_size):
            ...
            # 从dataset获取文件路径信息（兼容Subset）
            base_dataset = dataset.dataset if hasattr(dataset, 'dataset') else dataset
            actual_idx = dataset.indices[global_idx] if hasattr(dataset, 'indices') else global_idx
