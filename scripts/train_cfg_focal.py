#!/usr/bin/env python
"""
UNet 训练脚本（支持 CFG、CBCT初始化 和 Focal MSE Loss）

用法:
    # 训练带 Focal MSE Loss 的模型
    python scripts/train_cfg_focal.py --arch concatenation --epochs 200 \
        --use-cfg --gamma 2.0

    # 调整 gamma 参数（越大对难样本权重越高）
    python scripts/train_cfg_focal.py --arch concatenation --epochs 200 \
        --use-cfg --gamma 3.0

    # 快速测试
    python scripts/train_cfg_focal.py --arch concatenation --epochs 5 --quick-test \
        --use-cfg --gamma 2.0

CFG (Classifier-Free Guidance) 参数:
    --use-cfg: 启用CFG训练（条件丢弃）
    --cfg-dropout-rate: 条件丢弃率 (默认: 0.15)

Focal MSE Loss 参数:
    --gamma: Focal loss gamma参数，越大对难样本权重越高 (默认: 2.0，推荐 1.0-3.0)

CBCT初始化参数:
    --use-cbct-init: 使用CBCT初始化（从CBCT混合噪声开始扩散）
    --alpha-a: CBCT混合比例 (默认: 0.5)
"""
import os
import sys
import gc
...
def focal_mse_loss(pred, target, gamma=2.0):
    """
    Focal MSE Loss: 对误差大的区域施加更高权重

    用于改善骨头等高密度区域的生成质量，这些区域通常误差较大但样本量少。

    Args:
        pred: 预测噪声
        target: 真实噪声
        gamma: 聚焦参数，越大对难样本权重越高 (推荐 1.0-3.0)
               gamma=0 等价于标准 MSE
               gamma=2 对误差 0.5 的像素权重是误差 0.1 像素的 25x
    ...
    mse = (pred - target) ** 2
    # 误差大的地方权重更高
    weight = (mse.detach() + 1e-6) ** (gamma / 2)
    # 归一化保持 loss scale 稳定
...
def parse_args():
    ...
    parser.add_argument('--use-cfg', action='store_true',
                        help='启用CFG训练（condition dropout）')
    parser.add_argument('--cfg-dropout-rate', type=float, default=0.15,
                        help='条件丢弃率 (推荐: 0.10-0.20)')

    # CBCT初始化参数
    parser.add_argument('--use-cbct-init', action='store_true',
                        help='训练时使用CBCT初始化（从CBCT混合噪声开始扩散）')
    parser.add_argument('--alpha-a', type=float, default=0.5,
                        help='CBCT初始化混合比例 (0-1, 越大CBCT权重越高)')

    # Focal MSE Loss 参数
    parser.add_argument('--gamma', type=float, default=2.0,
                        help='Focal loss gamma参数，越大对难样本权重越高 (默认: 2.0，推荐 1.0-3.0)')
...
def train_with_wandb(
    ...
    # CFG状态
    if use_cfg:
        print(f"✓ CFG enabled (dropout_rate={cfg_dropout_rate})")
    else:
        print("✗ CFG disabled")

    # CBCT初始化仅用于推理
    if use_cbct_init:
        print(f"ℹ️  CBCT initialization will be used during inference (alpha_a={alpha_a})")
    else:
        print("ℹ️  Pure noise initialization for inference")

    # Focal MSE Loss
    print(f"✓ Focal MSE Loss enabled (gamma={gamma})")
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for ct, cbct in progress_bar:
            ...
            # 标准扩散加噪（训练时始终用CT作为目标）
            # 注意：CBCT初始化只在推理时使用，训练时用标准流程
            ct_z_noisy = diffusion.add_noise(ct_z, t, noise=noise)

            # CFG: Condition Dropout (训练时随机丢弃条件)
            if use_cfg:
                keep_mask = (torch.rand(cbct_z.size(0), 1, 1, 1, device=device) >= cfg_dropout_rate).float()
                cbct_z_input = cbct_z * keep_mask
            else:
                cbct_z_input = cbct_z

            if use_amp:
                with autocast(dtype=dtype):  # PyTorch 2.0兼容
                    pred_noise = unet(ct_z_noisy, cbct_z_input, t)
                    loss = focal_mse_loss(pred_noise, noise, gamma=gamma)
            else:
                pred_noise = unet(ct_z_noisy, cbct_z_input, t)
                loss = focal_mse_loss(pred_noise, noise, gamma=gamma)
        ...
        if predict_dir and (epoch + 1) % 20 == 0:
            generate_predictions(
                ...
                use_cbct_init=use_cbct_init, alpha_a=alpha_a
            )
...
def generate_predictions(unet, vae, test_loader, diffusion, device, epoch, predict_dir, wandb_logger, ema, num_samples=4, use_cbct_init=False, alpha_a=0.5):
    """
    生成预测样本并上传到 wandb

    Args:
        num_samples: 随机采样的样本数量
        use_cbct_init: 是否使用CBCT初始化
        alpha_a: CBCT初始化混合比例
    """
    import torchvision
    import random
    ...
    for sample_idx, data_idx in enumerate(random_indices):
        ...
        with torch.no_grad():
            ...
            if use_cbct_init:
                ...
                z = torch.sqrt(alpha_eff) * cbct_z + torch.sqrt(1 - alpha_eff) * torch.randn_like(cbct_z)
            else:
...
def train_architecture(arch_name, args, wandb_logger=None):
    ...
    # 生成带时间戳的实验目录名 (格式: arch_CFG_focal_timestamp_ep200)
    # 注意：CBCT初始化只在推理时使用，不影响训练目录命名
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    cfg_suffix = "CFG" if args.use_cfg else "noCFG"
    exp_dir_name = f"{arch_name}_{cfg_suffix}_focal_g{args.gamma}_{timestamp}_ep{args.epochs}"
...
def main():
    ...
    for arch in architectures:
        ...
        if not args.no_wandb:
            ...
            cfg_tag = "cfg" if args.use_cfg else "no-cfg"
            init_tag = "cbct-init" if args.use_cbct_init else "pure-noise"
            wandb_logger = WandbLogger(
                project=args.wandb_project,
                name=f"arch_{arch}_{cfg_tag}_{init_tag}",
                config={
                    "experiment_type": "architecture_comparison",
                    "architecture": arch,
                    "cfg": args.use_cfg,
                    "cfg_dropout_rate": args.cfg_dropout_rate if args.use_cfg else None,
                    "cbct_init": args.use_cbct_init,
                    "alpha_a": args.alpha_a if args.use_cbct_init else None,
                },
                tags=[arch, cfg_tag, init_tag, "architecture-comparison"],
            )
