...
- VAE 已完成全量训练验证：`checkpoints/vae/vae_best.pth` 在完整 val 集 2487 slices / 156 batches 上 `val_loss=0.252599`，无 NaN。该 VAE 是基于旧 train=199 split 训练的；由于 val 23 例保持不变，val 指标仍可横向比较，但尚未用 train=218 重训。
...
`SPLIT_COUNTS` (`scripts/preprocess_synthrad_dataset.py`) 现在是 2-tuple `(n_train_first, n_val)`：BB=(49,6)、AB=(44,5)、HN=(54,6)、TH=(52,6)，"前 49 + 后 5" 都归 train，与 VAE/Phase 1 已用过的 23 例 val 完全一致。VAE val loss 仍可比较；是否用 train=218 重训 VAE 是独立决策，不是当前 Phase 1 blocker。
...
旧显存估算表已删除：它低估了 UNet + ControlNet + PACA 反传激活，曾错误地给出 4090 上 batch 16-32 舒适的结论。

**实测结论**：RTX 4090 24GB 上，AMP 修复前 `base_channels=64` 且 `batch_size=8` 已在第一次 backward 发生 OOM（PyTorch reserved ~22.65GB，尝试再分配 4GB）。根因不是 DataLoader，而是训练循环只打印 `AMP Enabled: True`，没有实际使用 `autocast + GradScaler`。
...
Phase 1：统一基线
  ...
  模型：A0 = Concat latent diffusion + region embedding + mask-weighted loss
  条件：cbct_local / cbct_z
  附加：region embedding 注入 timestep embedding
  目标：建立最小可靠 baseline，作为 DR / ControlNet / PACA 增强模块的对照
```

Phase 1 baseline 以 `SPEC_experiments.md` 的 A0 为准。当前全组件版 `DR + ControlNet residual add + PACA` 是 C3 / D candidate，不再作为唯一 baseline。正式 VAE 和 Phase 1 长训稳定之前，不实现额外全局引导。

当前执行顺序：

1. 用 `vae_best.pth` 固定 VAE，先跑小规模 ConcatPACA Phase 1 训练，确认 loss 能下降、WandB 指标正常。**已完成**。
2. 按 `SPEC_experiments.md` 先实现并筛选 A0 / B1 / B2 / C1 / C2 / C3。
3. 用最佳组合启动 D1 长训。
4. 若合成 CT 出现明显床板/固定装置伪影，再做 mask outside air 或 mask-weighted ablation；当前不先硬改预处理主线。
