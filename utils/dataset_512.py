...
"""
专为512×512数据定制的数据集类
修复原始dataset.py中强制resize到256×256的问题
"""
...
class PairedCTCBCTDatasetNPY512(Dataset):
    """
    ...
    def __getitem__(self, idx):
        ...
        # 优先使用512×512路径
        if 'cbct_512_path' in row and row['cbct_512_path']:
        ...
        elif 'cbct_256_path' in row and row['cbct_256_path']:
...
class CTDatasetNPY512(Dataset):
    """
    专为512×512数据设计的单模态CT数据集（用于VAE训练）
    """
...
def get_dataloaders_512(manifest_path, batch_size, num_workers, 
                       dataset_class=PairedCTCBCTDatasetNPY512,
                       target_size=(512, 512),
    ...
    if val_size is not None and val_size < len(val_dataset):
        indices = torch.randperm(len(val_dataset))[:val_size]
