...
| 数据集 | 部位 | 训练集 | 官方验证集（无 GT） |
|--------|------|--------|------------------|
| synthRAD2023 | Brain (BB) | 61 例 | 11 例 |
| synthRAD2025 | Abdomen (AB) | 54 例 | 9 例 |
| synthRAD2025 | Head & Neck (HN) | 66 例 | 11 例 |
| synthRAD2025 | Thorax (TH) | 64 例 | 11 例 |
| **合计** | | **245 例** | **42 例（提交用）** |
...
[Step 7] 保存预处理后 volume 为 MHA
         ...
         保留空间元信息（spacing / origin），便于 QC 和未来 3D 扩展
```
...
preprocess = Compose([
    LoadImaged(keys=["ct", "cbct", "mask"]),
    EnsureChannelFirstd(keys=["ct", "cbct", "mask"]),
    ...
    CropForegroundd(                               # Step 3: mask crop
        keys=["ct", "cbct", "mask"],
        source_key="mask", select_fn=lambda x: x > 0, margin=10,
    ),
...
```
patient_id, region, split, ct_path,                           cbct_path,                          mask_path,                          cbct_global_path
2ABD100,    AB,     train, data/pp/2ABD100/ct_preprocessed.mha, data/pp/2ABD100/cbct_preprocessed.mha, data/pp/2ABD100/mask_preprocessed.mha, data/pp/2ABD100/cbct_global.mha
2BB002,     BB,     train, data/pp/2BB002/ct_preprocessed.mha,  data/pp/2BB002/cbct_preprocessed.mha,  data/pp/2BB002/mask_preprocessed.mha,  data/pp/2BB002/cbct_global.mha
```

- `region`：BB / AB / HN / TH（用于 region embedding）
- `cbct_global_path`：整体缩放的全局 CBCT volume，Phase 2 启用
...
## 三、数据加载（MONAI CacheDataset）

### 3.1 策略

预处理输出为 MHA volume，训练时用 **MONAI CacheDataset** 一次性缓存所有 volume 到内存，之后按 Z 轴提取切片：
...
class SliceDataset(Dataset):
    """从 CacheDataset 中的缓存 volume 展开为 2D slice。"""
    ...
    def __init__(self, manifest_csv, split, augmentation=None):
        ...
        volume_list = [
            {"ct": r.ct_path, "cbct": r.cbct_path, "mask": r.mask_path,
             "region": REGION_TO_ID[r.region]}
            for _, r in df.iterrows()
        ]
        self.cache_ds = CacheDataset(
            data=volume_list,
        ...
        self.index = []
        for vi, item in enumerate(self.cache_ds):
            mask = item["mask"][0]          # (Z, 256, 256)
    ...
    def __getitem__(self, idx):
        vi, z = self.index[idx]
        item = self.cache_ds[vi]
        ct   = item["ct"][0, z]     # (256, 256)
        cbct = item["cbct"][0, z]
        mask = item["mask"][0, z]
        region_id = torch.tensor(item["region"], dtype=torch.long)

        if self.augmentation:
            ct, cbct = self.augmentation(ct, cbct)

        return ct.unsqueeze(0), cbct.unsqueeze(0), mask.unsqueeze(0), region_id
```
...
**结论：256×256 2D 训练对任何 ≥16GB 显卡都完全够用**，模型主体在 64×64 latent 空间工作，显存瓶颈是 Adam 优化器状态（613M × 8B = 4.9GB），激活值相对模型权重仍是次要因素。
...
```
Phase 1：统一基线
  数据：BB + AB + HN + TH 全部 245 例，2D slice
...
Phase 2：全局引导 fine-tune（AB / TH 为主）
  额外输入：cbct_global（整体缩略图，Phase 1 预处理时同步生成）
  架构改动：UNet init_conv 从 in×2 → in×3（local + global concat）
  训练：Phase 1 权重热启，lr 降 5–10 倍
```

### 5.2 Region Embedding
...
loss_dr   = degradation_loss(intermediate_preds, ct_img)   # 已有 pixel-space mask 过滤
...
以下三处代码与 Spec 不符，需在正式训练前修改。

### 6.1 UNetConcatControlPACA：添加 Region Embedding

`models/unetConcatControlPACA.py` 的 `UNetConcatControlPACA.__init__` 中目前**无** `nn.Embedding`。
...
`models/unetConcatControlPACA.py` 中 `train_unet_concat_control_paca` 的 for 循环：
