...
**实测修正**：旧估算明显偏乐观。RTX 4090 24GB 上，`base_channels=64` 且 `batch_size=8` 已在第一次 backward 发生 OOM（PyTorch reserved ~22.65GB，尝试再分配 4GB）。小规模验证可用 `base_channels=64, batch_size=1` 稳定跑通。

因此 Phase 1 正式长训前需要先做显存优化或下调配置：

- 优先用 `batch_size=1` 或 `2` 起步，配合梯度累积实现等效 batch。
- `base_channels=256` 原始设定在单张 4090 上大概率不可直接训练。
- 后续应加入 gradient accumulation、activation checkpointing，或把正式 baseline 暂定为 `base_channels=64/128`。
