# [SynthRAD2025 项目笔记 2026-05-07] MAISI 3D-RFlow CBCT→CT 适配分析

## 结论（TL;DR）

**MAISI 3D-RFlow 可以接受 CBCT 图像作为 ControlNet 条件**，但需要工程适配。执行策略：**D1 continuation 立即保底 + MAISI CBCT→CT 并行探索（路径 C）**。

---

## 技术事实

### 1. MAISI 原生 ControlNet：掩码条件，非 raw 图像

MAISI 的 ControlNet 原生接受**分割掩码**（organ/tumor mask）作为条件：
- `controllable_anatomy_size`：解剖结构名称 + 大小比例
- MAISI-v2 论文（arXiv 2508.05772）明确："conditioning input c (e.g., segmentation mask) is processed through a separate encoder"

### 2. 参考实现证明可适配为 raw 图像输入

**`brudfors/maisi-mr-to-ct`**（NVIDIA 官方认可的社区实现）将 MAISI ControlNet 的掩码条件替换为 raw MR 图像：
- 只训练 ControlNet（72M / 273M 参数），**冻结 VAE + Diffusion UNet**
- GPU：推荐 24GB+（RTX 4090 满足）
- 单 subject 测试：~20 分钟，<12GB VRAM
- 完整 dataset 训练：多 GPU 需数天

### 3. CBCT→CT 平移可行性

| 对比项 | MR→CT（已有实现） | CBCT→CT（本项目） |
|---|---|---|
| 输入模态 | MR（任意单位） | CBCT（HU 值域，更接近 CT） |
| 配对数据 | 需要配对 MR-CT | SynthRAD2025 已提供配对 3D CBCT-CT |
| ControlNet 输入 | raw MR 体积 | raw CBCT 体积（直接替换） |
| 预期难度 | 参考基准 | 理论上更容易（同 HU 值域） |

---

## D1 vs MAISI CBCT→CT 对比

| 维度 | MAISI CBCT→CT | D1 继续训练 |
|---|---|---|
| **技术可行性** | ✅ 有 MR→CT 参考实现，平移可行 | ✅ 已验证，checkpoint 就绪 |
| **预期质量上限** | 更高（3D 上下文 + 大规模 CT 预训练） | ~25 HU（VAE floor） |
| **3D 上下文** | ✅ 真 3D 体积，无 slice inconsistency | ❌ 2D 切片独立处理 |
| **预训练知识** | ✅ NVIDIA 大规模 CT 数据预训练 | ❌ 仅 SynthRAD ~100 例 |
| **工程工作量** | ⚠️ 高：3D pipeline、权重下载、ControlNet 适配 | ✅ 低：直接 resume |
| **首个结果时间** | 数天～一周 | 数小时 |
| **GPU 内存** | ⚠️ 512×512×128 训练需 ~12GB（理论够，实测未知） | ✅ 2D bs24 稳定跑通 |
| **风险** | ⚠️ CBCT 配置未经测试 | ✅ 已知配置，可预期 |
| **论文新颖性** | 高（3D foundation model + CBCT adaptation） | 低（标准 2D LDM fine-tune） |

当前 D1 状态：VAE floor=25 HU，pilot MAE=101 HU，gap=76 HU → 扩散模型仍有大量提升空间。

---

## 执行计划（路径 C：并行）

### 立即：D1-stable-continuation 保底

```bash
python scripts/train_concat_paca.py \
  --use-controlnet --control-source cbct_latent --controlnet-fusion add \
  --base-channels 256 --use-ema --ema-decay 0.9995 \
  --lr 3e-5 --lr-schedule sd-warmup-constant --warmup-steps 1000 \
  --amp-dtype bf16 --batch-size 24 --epochs 100 \
  --sampler-init cbct --sampler-t-start 300 --sampler-alpha 1.0 \
  --resume checkpoints/phase1_matrix/D1-bf16-drop01-bc256-bs24-ep100-s42/unet_ema_state.pth
```

### 并行：MAISI CBCT→CT 适配（1~2 周探索）

1. 克隆 [NV-Generate-CTMR](https://github.com/NVIDIA-Medtech/NV-Generate-CTMR)，下载 rflow-ct 预训练权重
2. 参考 [brudfors/maisi-mr-to-ct](https://github.com/brudfors/maisi-mr-to-ct)，将 MR 替换为 CBCT 作为 ControlNet 输入
3. SynthRAD2025 数据已是 3D 配对体积（检查 spacing/preprocessing 是否匹配 MAISI 要求）
4. 单 subject 快速测试（<20 分钟），确认 pipeline 通畅
5. 全量训练：仅 ControlNet（72M 参数），冻结 VAE+UNet

### 约 1 周后决策点

- MAISI 单 subject MAE 明显优于 D1 pilot (101 HU) → 全量 MAISI 训练
- MAISI 踩坑无法稳定 → 放弃 MAISI，专注 D1 module ablation

### 论文价值分工

- **D1**：消融实验基线（ControlNet / region embedding 各自贡献的量化证据）
- **MAISI**：主方法（3D foundation model + CBCT adaptation → 更强结果 + 更高创新点）

---

## 参考资料

- [NV-Generate-CTMR](https://github.com/NVIDIA-Medtech/NV-Generate-CTMR) — 最新 MAISI 替代仓库（rflow-ct 权重）
- [brudfors/maisi-mr-to-ct](https://github.com/brudfors/maisi-mr-to-ct) — MR→CT 参考实现（ControlNet 接受 raw image）
- [MAISI-v2 论文 arXiv 2508.05772](https://arxiv.org/html/2508.05772v1) — rflow-ct 技术细节

---

以下为 NVIDIA 官方原始 README（此文件夹已弃用，最新版见 NV-Generate-CTMR）：
