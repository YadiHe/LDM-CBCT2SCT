...
def train_cond_unet(
    ...
    for epoch in range(epochs):
        ...
        for i, (ct, cbct) in enumerate(train_loader):
            ...
            # 🔥 条件丢弃机制（Conditional Dropout for CFG）
            # 批次中每个样本独立地以 15% 概率丢弃条件
            dropout_prob = 0.15
            # 生成 [batch_size, 1, 1, 1] 的 mask，每个样本独立随机
            keep_condition_mask = (torch.rand(cbct_z.size(0), 1, 1, 1, device=device) >= dropout_prob).float()
            # 应用 mask：被丢弃的样本条件变为零张量
            cbct_z_dropped = cbct_z * keep_condition_mask

            # 可选：每100个batch打印一次丢弃统计（方便调试）
            if i % 100 == 0:
                num_dropped = (keep_condition_mask == 0).sum().item()
                print(f"  Batch {i}: {num_dropped}/{cbct_z.size(0)} samples dropped condition")
            ...
            if use_fp16:
                with autocast():
                    ...
                    noise = torch.randn_like(ct_z)
                    ct_z_noisy = diffusion.add_noise(ct_z, t, noise=noise)
                    pred_noise = unet(ct_z_noisy, cbct_z_dropped, t)  # 使用丢弃后的条件
