...
class UNetConcatControlPACA(nn.Module):
    ...
    def forward(self, x, condition, t, down_paca_control_residuals=None, middle_paca_control_residual=None, down_control_residuals=None, middle_control_residual=None, region_id=None):
...
def train_unet_concat_control_paca(
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss_total < best_val_loss:
            ...
            # Full UNet — needed for resume / inference
            torch.save(unet.state_dict(), os.path.join(save_dir, "unet_full.pth"))
            # PACA-only slice — kept for compatibility with load_unet_concat_control_paca
            ...
            print(f"✅ Epoch {epoch+1} | val {avg_val_loss_total:.6f} | saved unet_full + controlnet + dr_module")
